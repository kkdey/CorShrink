...
ggcorrplot <- function (corr, method = c("square", "circle"),
  ...
  colnames(corr2) <- c("Var1", "Var2", "value")
  corr2$pvalue <- rep(NA, nrow(corr2))
  corr2$signif <- rep(NA, nrow(corr2))
  if (!is.null(p.mat)) {
    p.mat <- reshape2::melt(p.mat, na.rm = TRUE)
    corr2$coef <- corr2$value
    corr2$pvalue <- p.mat$value
    corr2$signif <-  as.numeric(p.mat$value <= sig.level)
    p.mat <- subset(p.mat, p.mat$value > sig.level)
    if (insig == "blank")
      corr2$value <- corr2$value * corr2$signif
  }

  corr2$abs_corr <- abs(corr2$value) * 10

  # Heatmap
  p <-
    ggplot2::ggplot(corr2, ggplot2::aes_string("Var1", "Var2", fill = "value"))
  if (method == "square") {p <- p + ggplot2::geom_tile(color = outline.color)
