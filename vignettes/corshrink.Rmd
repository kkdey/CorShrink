...
```{r corshrink_data, eval=TRUE, warning = FALSE, message=FALSE, fig.align = "left", fig.show="asis", dpi=144, fig.width=13, fig.height=10}
...
out1 <- CorShrinkData(sample_by_feature_data, sd_boot = FALSE, image = "output", 
                      image.control = list(title = "CorShrink (target = 0)", lab_size = 2))
out2 <- CorShrinkData(sample_by_feature_data, sd_boot = FALSE, image = "output", 
                     ash.control = list(mode = "estimate"),
                     image.control = list(title = "CorShrink (target = estimated)", lab_size = 2))

library(gridExtra)
grid.arrange(out1$image, out2$image, nrow = 1)
```
...
out3 <- CorShrinkData(sample_by_feature_data, sd_boot = FALSE, image = "output", 
                     ash.control = list(mixcompdist = "halfuniform"))
out4 <- CorShrinkData(sample_by_feature_data, sd_boot = FALSE, image = "output",  
                     ash.control = list(mixcompdist = "+uniform"))

library(gridExtra)
grid.arrange(out1$image, out2$image, out3$image, out4$image, nrow = 2)
```
