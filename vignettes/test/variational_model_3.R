...
mixVBEM2 = function(matrix_lik, prior, pi_init = NULL,
  ...
  if(is.null(nu1_init)){ nu1_init = nu_weight}
  if(is.null(nu2_init)){ nu2_init = nu_weight}

  param_init <- c(pi_init, log(nu1_init+1e-15, base=2),
                  log(nu2_init+1e-15, base=2))

  res = squarem(par=param_init,fixptfn=VBfixpoint2,
                objfn=VBnegpenloglik2,matrix_lik=matrix_lik,
                prior=prior, control=controlinput)
...
VBfixpoint2 = function(param_post, matrix_lik, prior){
  ...
  nu1_post = 2^(param_post[(k+1)]) - 1e-15
  nu2_post = 2^(param_post[(k+2)]) - 1e-15
